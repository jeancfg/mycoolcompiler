...
class ClassTable {
	...
	private String checkExpression(Expression expr) {
		if (expr instanceof plus || expr instanceof sub || expr instanceof mul
			...
			return m_arith(expr);
		else if (expr instanceof lt || expr instanceof leq || expr instanceof eq || expr instanceof comp
			...
			no_expr no_expr = (no_expr) expr;
			return no_expr.toString();
	...
	private void iterateOverFeatures(Features features) {
		...
		while (elements.hasMoreElements()) {
			...
			if (aux instanceof attr) {
				...
				reportError(aux,
						"[ERROR] Expected attribute or method instance");
		}
	}
	...
	private String m_bool(Object expr) {
		return null;
//		lt || expr instanceof leq || expr instanceof eq || expr instanceof comp
//		|| expr instanceof isvoid
//		
//		if (expr instanceof lt)
//			return plusExpr((plus) leq);
//		else if (expr instanceof sub)
//			return subExpr((sub) expr);
//		else if (expr instanceof mul)
//			return mulExpr((mul) expr);
...
//		return null;
	...
	private String m_dispatch(dispatch dispatch) {
		...
		boolean search = searchMethod(currentClass, dispatch.name.str);
